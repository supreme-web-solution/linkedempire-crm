...
class CampaignController extends Controller
{
     ...
     */
    private function mapLegacyCampaign(Campaign $campaign): array
    {
        return [
            'id' => $campaign->id,
            'name' => $campaign->name,
            'status' => strtolower((string) ($campaign->status ?: 'draft')),
            'sequence_type' => $campaign->sequence_type ?: 'custom',
            'legacy' => true,
        ];
    }

    private function findLegacyCampaign(int $campaignId, mixed $user): ?Campaign
    {
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Same `campaigns` table as the LinkedEmpire web CRM.
        $rows = Campaign::query()
            ->where('user_id', $user->id)
            ->orderByDesc('id')
            ->get()
            ->map(fn (Campaign $campaign) => $this->mapLegacyCampaign($campaign))
            ->values();

        $v2Campaigns = V2Campaign::query()
            ->where('user_id', $user->id)
            ->where('organization_id', $organizationId)
            ->latest('id')
            ->get();

        if ($v2Campaigns->isNotEmpty()) {
            $rows = $rows->concat($v2Campaigns->map(fn (V2Campaign $campaign) => array_merge(
                $campaign->toArray(),
                ['legacy' => false]
            )))->values();
        }
    ...
    public function updateStatus(Request $request, int $campaignId): JsonResponse
    {
        ...
        if (!$campaign) {
            $legacy = $this->findLegacyCampaign($campaignId, $user);
            if (!$legacy) {
                return response()->json(['message' => 'Campaign not found.'], 404);
            }

            $legacy->forceFill(['status' => strtolower($data['status'])])->save();

            return response()->json(['data' => $this->mapLegacyCampaign($legacy->fresh())]);
        }
    ...
    public function run(Request $request, int $campaignId, CampaignRunDispatcher $dispatcher): JsonResponse
    {
        ...
        if (!$campaign) {
            ...
            $launch = app(CampaignLaunchService::class);
            $result = $launch->launchFromLegacy($legacy, $user);

            if ($result['blocked'] ?? false) {
                return response()->json([
                    'message' => $result['message'] ?? 'Campaign could not be launched.',
                    'data' => [
                        'queued' => false,
                        'legacy' => true,
                        'blocked' => true,
            ...
            return response()->json([
                'data' => [
                    ...
                    'legacy' => true,
                    'campaign_id' => $legacy->id,
                    'v2_campaign_id' => $result['v2_campaign_id'] ?? null,
                    'status' => 'preparing',
                ],
            ], 202);
        }
