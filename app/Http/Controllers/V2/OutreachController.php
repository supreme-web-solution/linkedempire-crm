...
class OutreachController extends Controller
{
    ...
    private function respondAfterOutbound(V2Message $message, string $action): JsonResponse
    {
        $message->refresh();
        ...
        if ($status === 'failed') {
            return response()->json([
                'message' => (string) ($meta['error'] ?? 'Outreach action failed.'),
        ...
        if ($status === 'deferred') {
            return response()->json([
                'message' => 'Daily LinkedIn limit reached. This action was rescheduled for later.',
                'error_code' => 'outreach_deferred',
            ], 422);
        }
        ...
        return response()->json([
            'data' => [
                ...
                'provider_result' => $meta['provider_result'] ?? null,
            ],
        ]);
    }
    ...
    public function sendInvitation(Request $request): JsonResponse
    {
        ...
        return $this->respondAfterOutbound($message, 'invite');
    }
    ...
    public function startChat(Request $request): JsonResponse
    {
        ...
        return $this->respondAfterOutbound(
            $message,
            $conversation->provider_chat_id ? 'message' : 'start_chat',
        );
    }
    ...
    public function sendMessage(Request $request): JsonResponse
    {
        ...
        return $this->respondAfterOutbound($message, 'message');
    }
}
