...
class LeadController extends Controller
{
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        // ── Route: URL-based search or profile pasted in linkedin_url field ─────
        elseif (!empty($data['linkedin_url'])) {
            if (preg_match('~linkedin\.com/in/([^/?#]+)~i', $data['linkedin_url'])) {
                Log::debug('[Search] Profile URL via linkedin_url field', ['url' => $data['linkedin_url']]);
                try {
                    $profileData = $provider->getProfileByUrl($data['linkedin_url'], $unipileAccountId);
                    $elements = [$profileData];
                    $result = ['items' => $elements, 'mode' => 'profile_import'];
                } catch (\Throwable $e) {
                    Log::error('[Search] Profile-URL import failed', ['error' => $e->getMessage()]);
            ...
            } elseif (! UnipileProvider::isValidSearchUrl($data['linkedin_url'])) {
                return response()->json([
                    'message' => 'Paste a LinkedIn people search URL or a profile URL (linkedin.com/in/username).',
                    'error_code' => 'invalid_import_url',
                ], 422);
            } else {
    ...
    private function resolveAudienceMeta(array $data, User $user): array
    {
        ...
        if ($existing) {
            return [
                'name' => $name,
                'list_hash' => (string) $existing->list_hash,
            ];
        }

        do {
            $listHash = random_int(1111111, 9999999);
        } while (SnLeadList::query()->where('list_hash', $listHash)->exists());
     ...
     */
    private function mapSnLeadRow(SnLead $lead): array
    {
        $profileId = (string) ($lead->sn_lid ?: $lead->lid ?: '');
        ...
        return [
            ...
            'company_name' => null,
            'provider_profile_id' => $profileId,
            'public_identifier' => (string) ($lead->lid ?: ''),
            'email' => $lead->email,
            'location' => $lead->geolocation,
        ];
    }
     ...
     * @return array<string, mixed>
     */
    private function mapAudienceListRow(AudienceList $lead): array
    {
        $profileId = (string) ($lead->con_id ?: $lead->con_public_identifier ?: '');

        return [
            'id' => $lead->id,
            'full_name' => trim($lead->con_first_name.' '.$lead->con_last_name),
            'headline' => $lead->con_job_title,
            'company_name' => $lead->con_company_name,
            'provider_profile_id' => $profileId,
            'public_identifier' => (string) ($lead->con_public_identifier ?: ''),
            'email' => $lead->con_email,
            'location' => $lead->con_location,
        ];
    }

    /**
     * Read leads from legacy CRM tables (`sn_leads`, `audience_lists`).
     *
     * @return array<int, array<string, mixed>>
     */
    private function legacyLeadsForSource(User $user, string $sourceExternalId, int $limit): array
    {
        if ($sourceExternalId === '') {
            return [];
        }

        $snList = SnLeadList::query()
            ->where('user_id', $user->id)
            ->where('list_hash', $sourceExternalId)
            ->first();
        ...
        if ($snList) {
            return SnLead::query()
                ...
                ->limit($limit)
                ->get()
                ->map(fn (SnLead $lead) => $this->mapSnLeadRow($lead))
                ->filter(fn (array $row) => ($row['provider_profile_id'] ?? '') !== '' || ($row['public_identifier'] ?? '') !== '')
                ->values()
                ->all();
        }
        ...
        $audience = Audience::query()
            ->where('user_id', $user->id)
            ->where('audience_id', $sourceExternalId)
            ->first();
        ...
        if ($audience) {
            return AudienceList::query()
                ...
                ->limit($limit)
                ->get()
                ->map(fn (AudienceList $lead) => $this->mapAudienceListRow($lead))
                ->filter(fn (array $row) => ($row['provider_profile_id'] ?? '') !== '' || ($row['public_identifier'] ?? '') !== '')
                ->values()
                ->all();
        }
    ...
    public function listSnSources(Request $request): JsonResponse
    {
        ...
        foreach (Audience::query()->where('user_id', $user->id)->orderByDesc('updated_at')->get() as $audience) {
            $audienceId = (string) $audience->audience_id;
            if ($audienceId === '' || isset($seen[$audienceId])) {
                continue;
            }

            $seen[$audienceId] = true;
            $rows[] = [
                'source_external_id' => $audienceId,
                'source_name' => $audience->audience_name,
                'leads_count' => AudienceList::query()->where('audience_id', $audienceId)->count(),
                'last_imported_at' => $audience->updated_at,
            ];
        }
    ...
    public function listSnImportedLeads(Request $request): JsonResponse
    {
        ...
        if ($sourceExternalId !== '') {
            $legacyRows = $this->legacyLeadsForSource($user, $sourceExternalId, $limit);
            if ($legacyRows !== []) {
                return response()->json(['data' => $legacyRows]);
            }
        }
        ...
        return response()->json(['data' => $query->latest('id')->limit($limit)->get()]);
    }
