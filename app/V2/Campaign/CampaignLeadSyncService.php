...
class CampaignLeadSyncService
{
    ...
    {
        $firstKey = app(LegacyCampaignSequenceNormalizer::class)->firstExecutableNodeKey(
        ...
        V2CampaignLeadProgress::firstOrCreate(
            ...
            [
                'current_node_key' => 0,
                'next_node_key' => $firstKey,
                'run_status' => 0,
            ]
        );
    }

    public function resetProgressToStart(V2Campaign $campaign): void
    {
        $firstKey = app(LegacyCampaignSequenceNormalizer::class)->firstExecutableNodeKey(
            is_array($campaign->node_model) ? $campaign->node_model : [],
            is_array($campaign->link_model) ? $campaign->link_model : [],
        ...
        $campaign->campaignLeads()
            ->whereIn('status', ['running', 'pending', 'error'])
