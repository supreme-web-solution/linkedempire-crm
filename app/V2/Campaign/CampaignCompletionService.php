...
class CampaignCompletionService
{
    ...
    public function maybeFinish(V2Campaign $campaign, ?V2CampaignRun $run = null): bool
    {
        ...
        $legacyId = (int) (($campaign->meta ?? [])['legacy_campaign_id'] ?? 0);
        if ($legacyId > 0) {
            $legacy = Campaign::query()->find($legacyId);
            if ($legacy) {
                app(LegacyCampaignStatsService::class)->syncLegacyStatus($legacy, $campaign->fresh() ?? $campaign);
            }
        }
