...
class LeadPipelineService
{
    ...
    {
        $numericHash = is_numeric($listHash) ? (int) $listHash : null;
        if ($numericHash === null) {
            do {
                $numericHash = random_int(1111111, 9999999);
            } while (SnLeadList::query()->where('list_hash', $numericHash)->exists());
        }

        $list = SnLeadList::query()->firstOrCreate(
            [
                'list_hash' => $numericHash,
                'user_id' => $user->id,
            ],
            [
        ...
        V2LeadSource::query()->updateOrCreate(
            [
                ...
                'source_external_id' => (string) $numericHash,
            ],
            [
                'source_payload' => [
