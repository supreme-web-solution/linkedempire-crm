...
class ProcessCampaignLeadJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        $progress = V2CampaignLeadProgress::query()->firstOrCreate(
            ...
            [
                'current_node_key' => 0,
                'next_node_key' => $this->firstNodeKey($campaign),
                'run_status' => 0,
            ]
        );
    ...
    private function processLeadWithSlot(
        ...
        $nodes = $this->normalizedNodes($campaign);
        $nodeKey = (int) ($progress->next_node_key ?: $this->firstNodeKey($campaign));
     ...
     */
    private function normalizedNodes(V2Campaign $campaign): array
    {
        $normalizer = app(LegacyCampaignSequenceNormalizer::class);

        return $normalizer->normalize(
            is_array($campaign->node_model) ? $campaign->node_model : [],
            is_array($campaign->link_model) ? $campaign->link_model : [],
        );
    }

    private function firstNodeKey(V2Campaign $campaign): int
    {
        return app(LegacyCampaignSequenceNormalizer::class)->firstExecutableNodeKey(
            is_array($campaign->node_model) ? $campaign->node_model : [],
            is_array($campaign->link_model) ? $campaign->link_model : [],
        );
    }
