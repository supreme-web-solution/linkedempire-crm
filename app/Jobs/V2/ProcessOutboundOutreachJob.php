...
class ProcessOutboundOutreachJob implements ShouldQueue
{
    ...
    public function handle(ProviderManager $providerManager, OutreachPersistenceService $persistence, CampaignLinkedInGuard $linkedInGuard): void
    {
        ...
        try {
            ...
            if (($this->isAccountMismatch($exception) || $this->isChatNotFound($exception)) && $this->action === 'message') {
                try {
                    $this->retryAsStartChat($providerManager, $persistence, $message);
                    return;
    ...
    private function afterStartChat(V2Message $message, array $result): void
    {
        ...
        if ($conversation) {
            ...
            $accountId = (string) ($this->payload['_unipile_account_id'] ?? $this->payload['account_id'] ?? '');
            if ($accountId !== '') {
                $meta['unipile_account_id'] = $accountId;
            }
    ...
    private function isAccountMismatch(Throwable $exception): bool
    {
        if (! $exception instanceof UnipileException) {
            return str_contains(strtolower($exception->getMessage()), 'account mismatch');
        }

        $response = is_array($exception->context['response'] ?? null)
            ? $exception->context['response']
            : [];
        $type = (string) ($response['type'] ?? $exception->context['error_code'] ?? '');

        return str_contains($type, 'account_mismatch')
            || str_contains(strtolower($exception->getMessage()), 'account mismatch');
    }

    private function isChatNotFound(Throwable $exception): bool
    {
        ...
        return str_contains($detail, 'chat not found')
            || str_contains($message, 'resource not found');
    }

    private function retryAsStartChat(
        ...
        $conversation = V2Conversation::query()->find($this->conversationId);
        if ($conversation) {
            $persistence->clearProviderChatId($conversation);
            $conversation = $conversation->fresh() ?? $conversation;
        }

        $meta = is_array($conversation?->meta) ? $conversation->meta : [];
        $attendeeIds = Arr::get($this->payload, 'attendee_ids', []);
        if (! is_array($attendeeIds) || $attendeeIds === []) {
            $attendeeIds = Arr::get($meta, 'attendee_ids', []);
        }
        $attendeeIds = array_values(array_filter(array_map('strval', is_array($attendeeIds) ? $attendeeIds : [])));
        if ($attendeeIds === []) {
            throw new UnipileException(
                'Could not restart this chat after reconnecting LinkedIn. Try messaging again.',
                422,
            );
        }
        ...
        Log::info('[Outreach] Retrying stale chat as start_chat after account mismatch', [
            'user_id' => $this->userId,
            'conversation_id' => $this->conversationId,
            'message_id' => $this->messageId,
        ]);

        $result = $providerManager->messaging($providerKey)->startChat([
        ...
        $persistence->createProviderAuditEvent(
            ...
            'outbound_start_chat_retry_'.$this->messageId.'_'.time(),
            [
                'conversation_id' => $this->conversationId,
                'message_id' => $this->messageId,
                'result' => $result,
                'retried_from' => 'account_mismatch',
            ]
        );
    }
}
